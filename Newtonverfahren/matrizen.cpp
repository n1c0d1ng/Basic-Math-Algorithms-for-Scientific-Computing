...
// Definition der Freien Funktion:
...
Matrix operator+(const Matrix &A, const Matrix &B)
{
    ...
    {
        std::cout << "Die Dimensionen müssen übereinstimmen. \n";
        return Matrix(0, 0); // Funktion wird hier abgebrochen
    }
    Matrix result(A.rows, B.cols);
    for (int i = 0; i < A.rows; ++i)
...
{
    Matrix result(vector.rows, vector.cols);
    for (int i = 0; i < vector.rows; ++i)
    {
        for (int j = 0; j < vector.cols; ++j)
        {
            result.data[i][j] = scalar * vector.data[i][j];
        }
    }
    return result;
}

// Implementierung Skalarprodukt: u^T * v = sum(u_i * v_i)
double dot(const Matrix &u, const Matrix &v)
{
    if (u.rows != v.rows || u.cols != 1 || v.cols != 1)
    {
        std::cout << "Skalarprodukt nur für Vektoren gleicher Dimension möglich.\n";
        return 0.0; // Fehlerwert
    }
    double result = 0.0;
    for (int i = 0; i < u.rows; ++i)
    {
        result += u.data[i][0] * v.data[i][0];
    }
    return result;
}